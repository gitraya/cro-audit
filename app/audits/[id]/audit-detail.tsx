...
// How often to re-fetch the row while the audit is still running, and how long
// to keep polling before we back off and let the user re-check manually.
const POLL_MS = 2_500;
...
export function AuditDetail({ initialAudit }: { initialAudit: AuditRow }) {
  const [audit, setAudit] = useState(initialAudit);
  const [timedOut, setTimedOut] = useState(false);

  const isPending = audit.status === "queued" || audit.status === "running";

  // Fetch the latest audit row and merge it into state. No navigation and no
  // router.refresh() — React just re-renders the parts that changed in place.
  const refresh = useCallback(async () => {
    const supabase = createClient();
    const { data } = await supabase
      .from("audits")
      .select("*")
      .eq("id", initialAudit.id)
      .single();
